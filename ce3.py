    ...
    def _get_completion(self):
        ...
        try:
            ...
            if response.stop_reason == "tool_use":
                ...
                # Add the assistant's response with tool_use to conversation history
                ...
                if getattr(response, 'content', None) and isinstance(response.content, list):
                    # Count tool_use blocks to ensure matching tool_results
                    tool_use_blocks = [block for block in response.content if block.type == "tool_use"]
                    
                    # Execute each tool in the response content
                    for tool_block in tool_use_blocks:
                        result = self._execute_tool(tool_block)
                        ...
                        if isinstance(result, (list, dict)):
                            tool_result["content"] = result  # Keep structured data intact
                        else:
                            tool_result["content"] = str(result)  # Convert to string for text content
                        
                        tool_results.append(tool_result)
                    
                    if tool_results:
                        # Append tool results as content of a user message
                        self.conversation_history.append({
                            "role": "user",
                        ...
                        self.console.print("[red]No tool results generated despite 'tool_use' stop reason.[/red]")
                        return "Error: No tool results generated"
                else:
